...
export default function PlanningPage() {
  ...
  return <div className="w-full px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
    ...
    <div className="mt-6 grid items-start gap-6 xl:grid-cols-[280px_minmax(0,1fr)]">
          <section onDragOver={(e) => e.preventDefault()} onDrop={(e) => { const id = draggedId(e); if (id) unschedule(id); }} className="rounded-xl border border-slate-200 bg-slate-100 p-4 xl:sticky xl:top-4"><div className="flex items-center justify-between"><div><h2 className="font-semibold text-slate-950">Unscheduled backlog</h2><p className="mt-1 text-xs text-slate-500">Drag a project to a sprint</p></div><span className="rounded-full bg-white px-2.5 py-1 text-xs font-semibold text-slate-600">{backlog.length}</span></div><div className="mt-4 min-h-28 space-y-3 rounded-lg border-2 border-dashed border-slate-300 p-2">{backlog.length ? backlog.map((project) => <div key={project.id} draggable onDragStart={(e) => { e.dataTransfer.setData("text/project-id", project.id); e.dataTransfer.effectAllowed = "move"; }} className="cursor-grab active:cursor-grabbing"><ProjectCard project={project} onSchedule={() => schedule(project.id, 1)} /></div>) : <p className="p-4 text-center text-sm text-slate-500">All projects are scheduled. Drop a timeline bar here to return it.</p>}</div></section>
